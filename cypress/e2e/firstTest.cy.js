...
describe('Verify Register account and login', () => {
    ...
    it('Verify text link "Register account now', () => {
        ...
        cy.get('#byemail div.col-sm-6 > input[data-check="email"]').type(Cypress.env('agent_login'), {force: true});
        cy.get('#byemail input[name="password"]').type(Cypress.env('agent_password'), {force: true});
